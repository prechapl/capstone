...
class TwoUpEvents extends Component {
    ...
    componentDidMount() {
        ...
        if (!this.state.events.length || !this.props.assignedEvents.length) {
            ...
            axios.get(`https://capstone-api-server.herokuapp.com/api/events/assigned/${id}`)
                .then(res => res.data)
    ...
    render() {
        ...
        if (this.state.showingAssigned) {
            events = this.props.assignedEvents.filter(ev => ev.ownerId === relative.id);
            type = 'ASSIGNED';
        }
        else {
            events = this.state.events.filter(ev => ev.ownerId === this.props.user.id);
            type = 'MY EVENTS'
        }
        return (
            <View style={styles.container}>
                ...
                {events.length ? (
                    <EventList eventlist={events} type={type} />
                        ...
                        <Text style={{ margin: 10, padding: 10, textAlign: 'center' }}>No Events</Text>
                    )}
                <TouchableOpacity
                    style={styles.button}
                    onPress={() => {
...
export default withNavigation(connect(mapStateToProps, mapDispatchToProps)(TwoUpEvents));
